...
if ($sensor == "cam.foscam.black.motion") {
	...
	$sensor_id = "input_boolean.cam_foscam_black_motion";
} else {
	exit();
}
...
//curl -X POST -H "Content-Type: application/json" -H "x-ha-access: hasspassGislaved" -d "{\"state\":\"ON\"}"

//http://localhost:8123/api/states/binary_sensor.cam_foscam_black_motion

$url = "http://localhost/api/states/" . $sensor_id;
echo $url;
